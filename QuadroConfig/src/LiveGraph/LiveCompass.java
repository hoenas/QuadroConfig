...
public class LiveCompass extends Canvas {
	...
	public LiveCompass(Color backgroundColor, Color foregroundColor, Color rasterColor, boolean useRaster,
			int rasterWidth) {
	...
	public void update(int angle) {
		if (this.isVisible() && bufferstrategy == null) {
			...
			if (useRaster) {
				 ...
				 * g2.drawString("O", this.getWidth() - 20 , this.getHeight() /
				 * 2); g2.drawString("S", this.getWidth() / 2 , this.getHeight()
				 * - 20); g2.drawString("W", 10 , this.getHeight() / 2);
				 */
			}

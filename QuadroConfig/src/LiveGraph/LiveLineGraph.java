...
public class LiveLineGraph extends Canvas {
	...
	public void update() {
		...
		if (this.isVisible() && bufferstrategy == null) {
			...
			/*
			 * // neue Werte eintragen for(int i = 0; i < graphlist.size(); i++)
			...
			if (useRasterY) {
				...
				for (int i = 0; i < rasterLineCountY; i++) {
					g2.drawLine(0, i * this.getHeight() / rasterLineCountY, this.getWidth(),
							i * this.getHeight() / rasterLineCountY);
				}
			}
				...
				if (graph.isVisible()) {
					...
					for (int i = 0; i < graph.getBuffer().length - 1; i++) {
						...
						int y2 = this.getHeight()
								- (int) ((graph.getBuffer()[i + 1] + offset) * this.getHeight() / max);
